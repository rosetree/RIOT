...
static int _geschw_neu(int argc, char**argv)
{
	if (argc != 2) 
	{
        	printf("usage: %s <value> [mm] \n", argv[0]);
	        return 1;
    	}
	
	//VORBERECHNUNG

	int l_ges = atoi(argv[1]);
	int t_anfahrt = 2000; //Zeit zum Anfahren in ms
	int t_step = (int)(t_anfahrt/7); //Zeit die für jeden Anfahr-Schritt benötigt wird in ms
	...
	int l_anfahrt = (int)(0.004723888*t_step*U); //Länge, die bei der Anfahrt zurückgelegt wird in mm
	int l_mitte = (int)(l_ges - l_anfahrt*2); //Länge, die während der konstanten Periode zurückgelegt wird in mm
	int t_mitte = l_mitte / (0.000769231*U); // Zeit, die während der konstanten Periode vergeht in ms
...
static const shell_command_t shell_commands[] = { //Liste der Shell-Befehle
    ...
    {"test", "Länge einholen", _geschw_neu},
